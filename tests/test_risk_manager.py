...
from hongstr.execution.models import SignalEvent, OrderRequest, Position
...
def test_allow_long_enabled(risk_manager):
    ...
    sig = SignalEvent(ts=pd.Timestamp.now(), symbol="BTCUSDT", portfolio_id="HONG", 
                      strategy_id="test", direction="LONG", timeframe="1h", regime="TREND")
    req = OrderRequest(symbol="BTCUSDT", side="BUY", qty=0.1, order_type="MARKET", price=50000.0, extra={})
...
def test_max_notional_rejection(risk_manager):
    ...
    req = OrderRequest(symbol="BTCUSDT", side="BUY", qty=1.0, order_type="MARKET", price=50000.0, extra={})
...
def test_direction_disabled_short(risk_manager):
    ...
    sig = SignalEvent(ts=pd.Timestamp.now(), symbol="BTCUSDT", portfolio_id="HONG", 
                      strategy_id="test", direction="SHORT", timeframe="1h", regime="TREND")
    req = OrderRequest(symbol="BTCUSDT", side="SELL", qty=0.1, order_type="MARKET", price=50000.0, extra={})
