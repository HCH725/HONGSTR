                    ...
                    if "data" in record:
                        ...
                        if "ts" in record:
                            ...
                            if isinstance(record["ts"], int):
                                record["ts"] = pd.Timestamp(
                                    record["ts"], unit="ms", tz="UTC"
                                )
                            else:
                                record["ts"] = (
                                    pd.to_datetime(record["ts"]).tz_localize("UTC")
                                    if pd.to_datetime(record["ts"]).tz is None
...
def main():
    ...
    semantics = SemanticsV1(
        taker_fee_rate=args.fee_bps / 10000.0, slippage_bps=args.slippage_bps
    )
    ...
    for symbol in symbols:
        ...
        end_dt = (
            pd.Timestamp(args.end).tz_localize("UTC")
            if args.end != "now"
