...
readonly LABEL="com.hongstr.icloud_backup"
readonly PLIST_TEMPLATE="launchd/com.hongstr.icloud_backup.plist"
...
Usage:
  ...
  bash scripts/install_icloud_backup_launchd.sh reload
...
do_install() {
  ...
  echo "install_icloud_backup_launchd: installed ${plist_dest}"
...
do_uninstall() {
  ...
  launchctl bootout "gui/${uid}" "${plist_dest}" >/dev/null 2>&1 || true
  rm -f "${plist_dest}"
  echo "install_icloud_backup_launchd: removed ${plist_dest}"
}
...
main() {
  ...
  out_log="${repo_root}/_local/logs/launchd_icloud_backup.out.log"
  err_log="${repo_root}/_local/logs/launchd_icloud_backup.err.log"
