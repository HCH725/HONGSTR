...
def main():
    ...
    overfit_path = STATE_DIR / "overfit_governance_latest.json"
    overfit_data = read_json(overfit_path) or {}
    overfit_health = overfit_data.get("overall", "UNKNOWN")
    
    ssot_status = _collapse_system_status(
        [freshness_status, coverage_as_health, brake_status, regime_monitor_status, overfit_health]
    )
    ...
    system_health = {
        ...
        "components": {
            ...
            "overfit_governance": {
                "status": overfit_health,
                "candidates_seen": overfit_data.get("totals", {}).get("candidates_seen", 0),
                "pass": overfit_data.get("totals", {}).get("pass", 0),
                "fail": overfit_data.get("totals", {}).get("fail", 0),
            },
        },
        "sources": {
            ...
            "overfit_governance_latest.json": _source_meta(STATE_DIR / "overfit_governance_latest.json", now_ts),
        },
    }
