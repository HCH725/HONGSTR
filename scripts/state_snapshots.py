...
def _normalize_gate_status(status_raw: Any) -> str:
    ...
    if status in {"FAIL", "BLOCKED", "BLOCKED_DATA_QUALITY"}:
        return "FAIL"
    if status in {"UNKNOWN", "NEEDS_REBASE"}:
        return "UNKNOWN"
    return "UNKNOWN"
...
def _freshness_summary(freshness_table: dict[str, Any]) -> dict[str, Any]:
            ...
            offenders.append(
                {
                    ...
                    "profile": str(row.get("profile") or DEFAULT_FRESHNESS_PROFILE),
    ...
    offenders_sorted = sorted(
        offenders,
        key=lambda x: (
            float(x.get("age_h")) if _as_float_opt(x.get("age_h")) is not None else -1.0
        ),
        reverse=True,
    )
    return {
...
def _strategy_pool_top_entries(
    strategy_pool_data: dict[str, Any],
    strategy_pool_summary: dict[str, Any],
    top_n: int = 5,
) -> list[dict[str, Any]]:
    candidates = strategy_pool_data.get("candidates", []) if isinstance(strategy_pool_data, dict) else []
    ...
    legacy = strategy_pool_summary.get("leaderboard", []) if isinstance(strategy_pool_summary, dict) else []
    fallback: list[dict[str, Any]] = []
    for row in legacy if isinstance(legacy, list) else []:
        if not isinstance(row, dict):
            continue
        score = _as_float_opt(row.get("score"))
        sharpe = _as_float_opt(row.get("sharpe"))
        ret = _as_float_opt(row.get("return"))
        mdd = _as_float_opt(row.get("mdd"))
        metrics_status, reason = _metrics_status(
            {"score": score, "oos_sharpe": sharpe, "oos_return": ret}
        )
        fallback.append(
            {
                "strategy_id": row.get("id") or "unknown",
                "candidate_id": "",
                "family": "unknown",
                "direction": "UNKNOWN",
                "variant": "base",
...
def _research_leaderboard_top_entries(
    leaderboard_payload: dict[str, Any],
    top_n: int = 5,
) -> list[dict[str, Any]]:
    rows = leaderboard_payload.get("entries", []) if isinstance(leaderboard_payload, dict) else []
...
def _latest_backtest_head(
    leaderboard_payload: dict[str, Any],
    repo_root: Path,
) -> dict[str, Any]:
    entries = leaderboard_payload.get("entries", []) if isinstance(leaderboard_payload, dict) else []
    if not isinstance(entries, list) or not entries:
        return {
            "status": "UNKNOWN",
            "reason": "missing_research_leaderboard",
            "artifacts": {"report_dir": "", "summary": "", "gate": "", "selection": ""},
            "metrics": {"final_score": None, "oos_sharpe": None, "oos_mdd": None, "trades_count": None},
    ...
    chosen = max(
        (r for r in entries if isinstance(r, dict)),
...
def _governance_status(
    ...
    today_entries = [
        row
        for row in entries if isinstance(row, dict) and str(row.get("timestamp") or "").startswith(today)
    ]
    scope = "today_utc" if today_entries else "all_entries_fallback"
    target_rows = today_entries if today_entries else [r for r in entries if isinstance(r, dict)]
    counts = {"PASS": 0, "WARN": 0, "FAIL": 0, "UNKNOWN": 0}
    for row in target_rows:
        status = _normalize_gate_status(row.get("gate_overall"))
        if status not in counts:
            status = "UNKNOWN"
        counts[status] += 1
...
def _guardrails_summary(
    strategy_pool_data: dict[str, Any],
    loop_state: dict[str, Any],
) -> dict[str, Any]:
    report_only_value = None
    if isinstance(strategy_pool_data, dict):
        if isinstance(strategy_pool_data.get("report_only"), bool):
            report_only_value = strategy_pool_data.get("report_only")
    ...
    report_only_status = "PASS" if report_only_value is True else "UNKNOWN"
    actions_status = "PASS" if actions_empty else "UNKNOWN"

    overall = "PASS" if report_only_status == "PASS" and actions_status == "PASS" else "WARN"
    return {
        "status": overall,
        "checks": {
            "report_only_from_strategy_pool": {
                "status": report_only_status,
                "value": report_only_value,
            ...
            "core_diff_src_hongstr": {
                "status": "PASS_EXPECTED",
                "source": "preflight_required",
            },
            "tg_cp_no_exec": {
                "status": "PASS_EXPECTED",
                "source": "preflight_required",
...
def _build_daily_report_payload(
    *,
    ...
    top_pool = _strategy_pool_top_entries(strategy_pool_data, strategy_pool_summary, top_n=5)
    top_research = _research_leaderboard_top_entries(research_leaderboard, top_n=5)
    ...
    return {
        ...
        "research_leaderboard": {
            ...
            "top_entries": top_research,
        },
        "governance": _governance_status(research_leaderboard, policy_payload, now_utc),
...
def main():
    ...
    daily_report_payload = _build_daily_report_payload(
        ...
        strategy_pool_summary=pool_summary,
