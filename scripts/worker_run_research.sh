#!/usr/bin/env bash
# HONGSTR worker: continuous research loop runner (report_only)
set -euo pipefail

if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "[worker_research] macOS only. Detected: $(uname -s)" >&2
  exit 2
fi
...
RESEARCH_DRY_RUN="${HONGSTR_WORKER_RESEARCH_DRY_RUN:-0}"

mkdir -p "$STATE_DIR"
HEARTBEAT_PATH="$STATE_DIR/worker_heartbeat.json"
LAST_RUN_PATH="$STATE_DIR/last_run_research.json"
LOG_PATH="$STATE_DIR/research_worker.log"

if [[ ! -x "./.venv/bin/python" ]]; then
  echo "[worker_research] missing .venv; run bootstrap first" >&2
  exit 1
fi
...
write_last_run() {
  ...
  local git_sha="$6"
  local branch="$7"
  ./.venv/bin/python - "$LAST_RUN_PATH" "$start_utc" "$end_utc" "$exit_code" "$status" "$reason" "$git_sha" "$branch" <<'PY'
import json
import sys
from pathlib import Path
...
payload = {
    ...
    "report_only": True,
    "artifacts": {
        "research_state": "data/state/_research/loop_state.json",
        "research_leaderboard": "data/state/_research/leaderboard.json",
    },
}
path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
PY
...
run_once() {
  ...
  write_heartbeat "running" "RUN" "research loop execution started"

  sync_main_branch
  git_sha="$(git rev-parse HEAD 2>/dev/null || echo unknown)"
  branch="$(git branch --show-current 2>/dev/null || echo unknown)"

  local -a cmd=(bash scripts/run_research_loop.sh --once)
  if [[ "$RESEARCH_DRY_RUN" == "1" ]]; then
    cmd+=(--dry-run)
  fi

  {
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] run: ${cmd[*]}"
    "${cmd[@]}"
  } >>"$LOG_PATH" 2>&1 || rc=$?
  ...
  if [[ "$rc" -ne 0 ]]; then
    status="WARN"
    reason="research loop returned non-zero"
  fi

  write_last_run "$start_utc" "$end_utc" "$rc" "$status" "$reason" "$git_sha" "$branch"
  write_heartbeat "running" "$status" "research run finished in $((end_epoch - start_epoch))s"
  echo "[worker_research] run status=$status rc=$rc duration=$((end_epoch - start_epoch))s"
}
