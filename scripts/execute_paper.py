...
def generate_markdown_report(path: Path, data: Dict):
    ...
    if data.get("orders"):
        ...
        lines.append(
            "| Symbol | Side | Status | Qty | AvgPrice | OrderId | ClientOrderId |"
        )
        lines.append("|---|---|---|---|---|---|---|")
...
def main():
    ...
    parser.add_argument(
        "--run_dir", type=str, help="Manually specify run directory with selection.json"
    )
    parser.add_argument(
        "--send", action="store_true", help="Actually send orders (dry-run by default)"
    )
    parser.add_argument(
        "--max_notional_usd",
        type=float,
        default=50.0,
        help="Max notional allowed per order",
    )
    ...
    parser.add_argument(
        "--force_trade",
        action="store_true",
        help="Force a TRADE execution regardless of selection (TESTNET ONLY)",
    )
    parser.add_argument(
        "--force_side",
        type=str,
        choices=["BUY", "SELL"],
        default="BUY",
        help="Side for forced trade",
    )
    parser.add_argument(
        "--force_symbol", type=str, default="BTCUSDT", help="Symbol for forced trade"
    )
    parser.add_argument(
        "--force_notional_usd",
        type=float,
        default=10.0,
        help="Notional for forced trade",
    )
    parser.add_argument(
    ...
    sel_path = (
        Path(args.run_dir) / "selection.json"
        if args.run_dir
        else get_latest_selection(data_dir)
    )
    if not sel_path or not sel_path.exists():
        if args.force_trade:
            ...
            sel_data = {
                "decision": "HOLD",
                "regime": "FORCED",
                "selected_symbol": args.force_symbol,
            }
        else:
    ...
    if args.force_trade:
        is_testnet = (
            os.getenv("BINANCE_FUTURES_TESTNET") == "1"
            or os.getenv("BINANCE_TESTNET") == "1"
        )
        if not is_testnet:
            print(
    ...
    if decision == "TRADE":
        ...
        if notional_usd > args.max_notional_usd:
            ...
            try:
                r_p = requests.get(
                    f"{broker.base_url}/fapi/v1/ticker/price",
                ...
                if res.exchange_order_id:
                    print(
                        f"Order Sent: {res.exchange_order_id}. Fetching full details..."
                    )
