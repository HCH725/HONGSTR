...
- `regime_window_utc`: 該環境所定義的時間區間（格式固定 `["start_utc","end_utc"]`，語義為 `[start,end)`，UTC，end-exclusive）。
- `slice_rationale`: 切片採用理由（人類可讀；可中文）。
- `fallback_reason`: 若無法匹配特定環境時的降級原因（通常降級為 `ALL` 全時段觀測；建議中文口語）。
