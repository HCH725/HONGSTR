...
- if decision is `upgrade-review pass`, the usual next action is `open review PR`
- if decision is `upgrade-review fail`, the usual next action is `continue observation`, `open retirement PR`, or `no action`
