...
def _build_selection(summary: dict[str, Any], gate_payload: dict[str, Any]) -> dict[str, Any]:
    passed = bool(gate_payload.get("overall") == "PASS")
    return {
        "decision": "SELECT" if passed else "WATCHLIST",
...
def _render_candidate_report(
    *,
    ...
    gate_status = "PASS" if gate_payload.get("overall") == "PASS" else "FAIL"
    reasons = gate_payload.get("reasons", [])
...
def _write_strategy_pool(records: list[dict[str, Any]]) -> Path:
    ...
    promoted = [r["candidate_id"] for r in ranked if r.get("gate_overall") == "PASS"][:3]
    demoted = [r["candidate_id"] for r in ranked if r.get("gate_overall") != "PASS"]
    payload = {
...
def _write_weekly_governance(records: list[dict[str, Any]]) -> tuple[Path, Path]:
    now = datetime.now(timezone.utc)
    iso_year, iso_week, _ = now.isocalendar()
    gov_dir = REPORTS_ROOT / "governance" / f"{iso_year}_W{iso_week:02d}"
    gov_dir.mkdir(parents=True, exist_ok=True)

    ranked = sorted(records, key=lambda x: float(x.get("last_score", 0.0)), reverse=True)
    promote = [r["candidate_id"] for r in ranked if r.get("gate_overall") == "PASS"][:3]
    demote = [r["candidate_id"] for r in ranked if r.get("gate_overall") == "FAIL" and float(r.get("last_score", 0.0)) < 90.0]
    watchlist = [
        r["candidate_id"]
        for r in ranked
        if r["candidate_id"] not in promote and r["candidate_id"] not in demote
    ][:8]

    policy_ref = "research/policy/overfit_gates_aggressive.json"
    governance_json = {
        ...
        "counts": {
            ...
            "pass": sum(1 for r in records if r.get("gate_overall") == "PASS"),
            "fail": sum(1 for r in records if r.get("gate_overall") == "FAIL"),
        },
        "recommendations": {
            "promote": promote,
            "demote": demote,
    ...
    json_path = gov_dir / "weekly_governance.json"
    json_path.write_text(json.dumps(governance_json, indent=2), encoding="utf-8")

    md_lines = [
        f"# Weekly Overfit Governance ({iso_year}-W{iso_week:02d})",
        "",
        "- report_only: true",
        f"- policy: `{policy_ref}`",
        f"- total_candidates: {governance_json['counts']['total_candidates']}",
        f"- pass/fail: {governance_json['counts']['pass']}/{governance_json['counts']['fail']}",
        "",
        "## Recommendations",
        ...
        "",
        "## Top Candidates",
        "| candidate_id | family | direction | score |",
        "|---|---|---|---:|",
    ]
    for row in governance_json["top_scores"]:
        md_lines.append(
            f"| {row['candidate_id']} | {row['family']} | {row['direction']} | {float(row['score']):.2f} |"
        )
    md_lines.extend(["", "Generated by research_loop (report_only)."])

    md_path = gov_dir / "weekly_governance.md"
...
def main() -> None:
    ...
    try:
        ...
        catalog = build_candidate_catalog(max_candidates=max_candidates)
        if not catalog:
        ...
        for idx, candidate in enumerate(catalog, start=1):
            ...
            proposal_data = {
                ...
                "parameters": candidate.get("parameters", {}),
                "metrics_to_watch": ["oos_sharpe", "oos_mdd", "trades_count"],
            ...
            gate_payload = {
                ...
                "hard_failures": gate_result.details.get("hard_failures", []),
                "soft_adjustments": gate_result.details.get("soft_adjustments", {}),
                "final_score": gate_result.details.get("final_score", 0.0),
                "policy_name": gate_result.details.get("policy_name", "unknown"),
                "report_only": True,
                "timestamp": summary.get("timestamp"),
            }
        ...
        weekly_md, weekly_json = _write_weekly_governance(records)

        top = sorted(records, key=lambda r: float(r.get("last_score", 0.0)), reverse=True)[0]
        top_pass = bool(top.get("gate_overall") == "PASS")

        write_state(
            status="OK" if not args.dry_run else "DRY_RUN",
            exp_id=str(top.get("candidate_id")),
            gate_passed=top_pass,
            report_path=str(representative_report or weekly_md),
        )

        notify_telegram(top, top_pass, representative_report or weekly_md, "OK" if not args.dry_run else "DRY_RUN")

        logger.info(
            "Research Loop v2 completed: candidates=%s pool=%s board=%s governance=%s,%s",
            len(records),
            pool_path,
            board_path,
            weekly_md,
