...
def test_dca_sweep_generates_param_variants():
    ...
    safety_mult_set = {float(c["parameters"].get("safety_multiplier", -1)) for c in catalog}
    safety_gap_set = {float(c["parameters"].get("safety_gap_bps", -1)) for c in catalog}
    assert len(safety_mult_set) >= 2
    assert len(safety_gap_set) >= 2
